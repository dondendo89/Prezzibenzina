...
export default function Lista({ items }: { items: Item[] }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {items.map((i) => (
        <a key={i.id} href={`/distributore/${i.id}`} target="_blank" rel="noreferrer" style={{ border: '1px solid #eee', padding: 8, borderRadius: 8, display: 'flex', justifyContent: 'space-between' }}>
          <div>
